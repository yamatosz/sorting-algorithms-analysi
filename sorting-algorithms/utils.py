...
def generate_lists(sizes, num_runs):
    ...
    for size in sizes:
        ...
        lists["sorted"][size] = [sorted(lists["random"][size][i]) for i in range(num_runs)]
        lists["reversed"][size] = [sorted(lists["random"][size][i], reverse=True) for i in range(num_runs)]
    return lists
